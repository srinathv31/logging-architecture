...
If the starter or test need **new SDK code** in the same release, use the SNAPSHOT workflow:

```
1. PR #1: SDK changes only (sub-deps at previous release)
   → merge → SNAPSHOT auto-publishes

2. PR #2: update sub-deps to current SNAPSHOT
   + starter/test changes that use new SDK code
   → PR check resolves SNAPSHOT from remote ✓
   → merge

3. Trigger release → everything publishes in sync
```
