...
describe("ProcessDefinitionService - Test Fixtures", () => {
  describe("createProcessFixture", () => {
    it("should create a valid fixture with default values", () => {
      ...
      expect(fixture.processName).toBe("test-process");
      expect(fixture.displayName).toBe("Test Process");
      expect(fixture.description).toBe("A test process for unit testing");
      expect(fixture.owningTeam).toBe("test-team");
      expect(fixture.expectedSteps).toBe(5);
      expect(fixture.slaMs).toBe(30000);
    });

    it("should allow overriding default values", () => {
      const fixture = createProcessFixture({
        processName: "custom-process",
        displayName: "Custom Process",
        expectedSteps: 10,
      });

      expect(fixture.processName).toBe("custom-process");
      expect(fixture.displayName).toBe("Custom Process");
      expect(fixture.expectedSteps).toBe(10);
      expect(fixture.owningTeam).toBe("test-team");
    });
    ...
    it("should have required fields", () => {
      ...
      expect(fixture).toHaveProperty("displayName");
      expect(fixture).toHaveProperty("description");
      expect(fixture).toHaveProperty("owningTeam");
    });
  });
...
describe("ProcessDefinitionService - createProcess behavior", () => {
  it("should map camelCase input to camelCase for DB insert", () => {
  ...
  it("should handle optional fields as undefined", () => {
    const input = createProcessFixture();
    // @ts-expect-error - testing that expectedSteps is optional
    delete input.expectedSteps;
    // @ts-expect-error - testing that slaMs is optional
    delete input.slaMs;
